...
function initHeroAnimations() {
    ...
    tl.from(".hero-background-image", {
        ...
        .from(".hero-subtitle", {
            ...
            y: 30,
            duration: 0.6
        }, "<0.2") // Start 0.2s after the title animation begins
        .from(".intro-text", {
            opacity: 0,
            y: 20,
            duration: 0.6
        }, "<0.2") // Start 0.2s after subtitle animation begins
        .from(".hero-buttons a", {
            opacity: 0,
            y: 20,
            duration: 0.5,
            stagger: 0.15 // Staggered animation for each button
        }, "<0.2") // Start 0.2s after intro text animation begins
        .from(".hero-shape.shape-1", {
            opacity: 0,
            x: -100,
            rotation: 45,
            duration: 1
        }, "0.5") // Start at 0.5s mark of the timeline
        .from(".hero-shape.shape-2", {
            opacity: 0,
            x: 100,
            rotation: -45,
            duration: 1
        }, "<0.2") // Start 0.2s after shape-1 animation begins
        .to(".scroll-indicator", {
            opacity: 1,
            y: 0,
            duration: 0.8,
            ease: "power2.out"
        }, "<0.5"); // Start 0.5s after shape-2 animation begins
